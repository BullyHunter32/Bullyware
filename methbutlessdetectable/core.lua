local chars = {"B","U","L","L","Y","W","A","R","E",".","_","-","L","E","S","S","D","E","T","E","C","T","A","B","L","E"}
local function generateStr( iLength )
    local str = ""
    for i=1, iLength do str = str .. chars[math.random(1,#chars)] end
    return str
end
local randomStr = generateStr(math.random(5,15))
_G[randomStr] = {}
local methamphetamine = _G[randomStr]
file.Write("!!!!!!!!"..generateStr(15) .. ".txt",randomStr)
methamphetamine.hooks = {
    ["log"] = generateStr(15),
    ["configLoaded"] = generateStr(15), 
    ["masterToggle"] = generateStr(15),
}
methamphetamine.hook,methamphetamine.timer = include("hook.lua")
...
methamphetamine.Log = function (text)
    ...
    hook.Run(methamphetamine.hooks.log, tbl )
end
...
function PANEL:Init()
    ...
    self.menubar.version:SetText("Bullyware Version 1.0.3" or "Methamphetamine Solutions v3.6.7")
...
hook.Add( "PlayerButtonDown", methamphetamine.hooks.masterToggle, function( ply, key )
    if key == KEY_0 then 
        ...
        if CurTime() - (frame.lastopen or 1) > 0.4 then
            ...
            hook.Run(methamphetamine.hooks.masterToggle,true)
